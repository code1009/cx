...
void net_msg_pool::cleanup(void)
{
	...
	for (i = 0; i < count; ++i)
	{
		...
		{
			// 올수 없는 조건
			std::cerr << "net_msg_pool::cleanup(): error!" << std::endl;
		}
	}
...
#if 1==net_msg_pool_debug
	std::cerr << "net_msg_pool::cleanup(): count=" << _count << " max_count=" << _max_count << std::endl;
#endif
	...
	sprintf_s(message, "net_msg_pool::cleanup(): count=%zu max_count=%zu \r\n", _count, _max_count);
...
net_msg* net_msg_pool::acquire(void)
{
	...
	std::cerr << "net_msg_pool::acquire(): empty!" << std::endl;
...
void net_msg_pool::release(net_msg* ptr)
{
	...
	{
		std::cerr << "net_msg_pool::release(): nullptr!" << std::endl;
	}
}
