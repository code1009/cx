...
/////////////////////////////////////////////////////////////////////////////
//===========================================================================
DWORD WINAPI _rs_comm_port_writer_thread_proc(LPVOID param)
{
	rs_comm_port_writer* instance = reinterpret_cast<rs_comm_port_writer*>(param);
...
/////////////////////////////////////////////////////////////////////////////
//===========================================================================
rs_comm_port_writer::rs_comm_port_writer() :
	_core(nullptr),
...
bool rs_comm_port_writer::create(void)
{
	if (nullptr == _core)
	{
		return false;
	...
	_io_event_handle = CreateEvent(nullptr, FALSE, FALSE, nullptr);
	...
	_thread_handle = CreateThread(0, 0, _rs_comm_port_writer_thread_proc, this, 0, &thread_id);
	if (nullptr == _io_event_handle)
	{
		return false;
	}
...
void rs_comm_port_writer::destroy(void)
{
	if (_thread_handle)
	{
		DWORD dwObject;
...
void rs_comm_port_writer::set_state(state s)
{
	_state = s;
}

void rs_comm_port_writer::thread_loop(void)
{
	bool loop;
	event e;
...
rs_comm_port_writer::event rs_comm_port_writer::wait_event(void)
{
	event e;
...
rs_comm_port_writer::event rs_comm_port_writer::wait_event_state_ready(void)
{
	event e;
...
rs_comm_port_writer::event rs_comm_port_writer::wait_event_state_io(void)
{
	event e;
...
rs_comm_port_writer::event rs_comm_port_writer::wait_event_state_io_ov(void)
{
	event e;
...
bool rs_comm_port_writer::on_event(event e)
{
	bool result;
	...
	case event::reset_buffer:
		on_event_reset_buffer();
		break;
...
void rs_comm_port_writer::on_event_ready(void)
{
	set_state(state::ready);
}

void rs_comm_port_writer::on_event_reset_buffer(void)
{
	_core->reset_tx();
...
void rs_comm_port_writer::on_event_io_open(void)
{
	set_state(state::io);
}

void rs_comm_port_writer::on_event_io(void)
{
	_core->pop_tx(_buffer);
	//ResetEvent(_io_event_handle);
	...
	rv = _core->_comm.write(_io_buffer, _io_size, &_io_transferred, &_io_ov);
...
void rs_comm_port_writer::on_event_io_ov(void)
{
	BOOL rv;
	...
	if (_io_transferred != _io_size)
	{
		CX_RUNTIME_LOG(cxLError) << L"GetOverlappedResult() : " << _io_transferred << L" / " << _io_size;
	}
...
void rs_comm_port_writer::on_event_io_close(void)
{
	BOOL rv;
...
void rs_comm_port_writer::on_event_io_exit(void)
{
	BOOL rv;
